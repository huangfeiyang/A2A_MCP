...
本仓库采用 **src 布局**：所有可运行代码都位于 `src/` 下；根目录只放配置、脚本与文档。整体分为三层：  
...
- `requirements.txt`：运行依赖（FastAPI/uvicorn/openai/httpx/pydantic 等）。
- `requirements-dev.txt`：开发依赖（pytest/ruff/black/mypy 等，建议开发时安装）。
...
- `scripts/smoke_test.sh`：冒烟测试（检查 `/agent-card` + `/v1/ask` 是否可用）。

---

### src (主代码包：唯一真实代码来源)

- 采用 src 布局：所有 import 从 `src/` 下开始。
- 当前主包结构：`src/tool_server/`、`src/agent_server/`、`src/client/`。
...
- `tool_server/settings.py`：工具服务配置读取（API keys、超时等；显式加载 `.env`）。
...
- `tool_server/adapters/amap.py`：高德 API 封装（POI + geocode）。
- `tool_server/adapters/openweather.py`：OpenWeather API 封装（同上）。

**Tools（薄工具 / 只做能力供给）**
- `tool_server/tools/time.py`：当前时间（纯函数，建议优先实现用于验证链路）。
- `tool_server/tools/weather.py`：天气查询（支持中文城市名映射与 geocode → 经纬度 → OpenWeather）。
- `tool_server/tools/poi.py`：POI 查询（city → geocode → around → 结构化 POI 列表）。
...
- `agent_server/agent.py`：智能体核心（单智能体 tool-use loop，多轮工具调用）：
  - 构建 messages
  - 调 OpenAI（tools/function calling）
  - 解析 tool_calls
  - 调 `tool_broker`
  - 回灌工具结果并产出最终回答
- `agent_server/tool_broker.py`：工具调用统一入口（async + httpx）：
  - 统一超时/错误归一
  - 记录每次工具调用耗时与 trace
  - 默认禁用系统代理（避免 localhost 502）
...
- `agent_server/settings.py`：Agent 配置（OpenAI key、模型名、MCP base url、max_tool_calls、超时、trace 等）。
...
- `client/cli.py`：命令行客户端（发送 query → 打印回答；`--verbose` 可打印 tool_calls/trace_id；可调整超时）。
...
- `tests/test_smoke_cli.py`：端到端冒烟测试（mock 模式、不依赖外部 API）。

### Smoke Test 说明

**目的**：确认最小链路可用（Agent 接口可访问 + 能返回答案）。  
**特点**：不依赖外部 API（使用 mock 模式），只验证“能跑通”。  
**执行方式**：
- 脚本：`scripts/smoke_test.sh`
- 测试用例：`PYTHONPATH=src pytest -q tests/test_smoke_cli.py`
